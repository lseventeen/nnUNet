...
def get_dice_hd95(pre_path,experiment_id, task_id):
    ...
    for i in range(len(dataset['evaluationClass'])):
        print('Mean class %s mean_dice %f mean_hd95 %f' % (clm[i+2], metric_list[i][0], metric_list[i][1]))
    ...
    data = np.hstack((performance*100,mean_hd95,metric_list[:,0]*100)).reshape(1,len(dataset['evaluationClass'])+2)

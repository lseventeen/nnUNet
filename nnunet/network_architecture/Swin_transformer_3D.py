...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # cyclic shift
        if self.shift_size > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size, -self.shift_size), dims=(1, 2))
        else:
            shifted_x = x
...
class PatchEmbed(nn.Module):
    ...
    def flops(self):
        So, Ho, Wo = self.patches_resolution
        flops = So * Ho * Wo * self.embed_dim * self.in_chans * (self.patch_size[0] * self.patch_size[1] * self.patch_size[2])
        if self.norm is not None:
            flops += So * Ho * Wo * self.embed_dim
        return flops
...
class SwinTransformer_3D(nn.Module):
    ...
    def __init__(self, img_size=[192,192,48], patch_size=[1,2,2], in_chans=1, num_classes=14,
